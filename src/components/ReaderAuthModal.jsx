...
export default function ReaderAuthModal({ onClose }) {
    ...
    const submit = async (event) => {
        ...
        try {
            ...
            if (mode === 'forgot') { await resetPassword(email); setMessage('Check your inbox for a password reset link.'); }
            else if (mode === 'signup') { await signUp({ email, password, displayName }); setMessage('Account created. Check your inbox if email confirmation is enabled.'); }
    ...
    return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm" role="dialog" aria-modal="true">
        <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0d0d12] p-6 shadow-2xl">
            ...
            <form className="space-y-4" onSubmit={submit}>
                {mode === 'signup' && <input required minLength={2} maxLength={80} value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="Display name" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none" />}
                ...
                {mode !== 'forgot' && <input required minLength={8} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password (8+ characters)" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none" />}
